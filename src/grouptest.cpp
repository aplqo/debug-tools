...
const std::array<const char*, 12> ResultHeader{
    "Id",     "State(Run)", "State(Test)", "Input",  "Output",      "Answer",
    "Differ", "Real",       "User",        "System", "Memory(MiB)", "Details"};
...
const std::array<const char*, 8> GroupHeader{
    "Id",        "InDir",  "AnsDir",      "Time",
    "Hard time", "Memory", "Hard memory", "Verbose"};
typedef Table::Table<8> GroupTable;

class TestPoint : public TestcaseType {
 public:
  TestPoint(const unsigned int id, const unsigned int gid, fs::path&& input,
            fs::path&& answer, TestTemplate& te)
      : TestcaseType(std::move(input), std::move(answer), te), id(id), gid(gid)
  {
  }
...
{
  std::cout << Escape::Underline << Escape::TextBlue
            << "[Info] Start program for test #" << gid << "." << id
            << Escape::None << "\n";
  if (verbose) printRunInfo(std::cout);
  std::cout.flush();
  run();
  ...
  if (runPass && !tester.path.empty()) {
    std::cout << Escape::TextBlue << "[Info] Start testing for test #" << gid
              << "." << id << Escape::None << "\n";
    if (verbose) printTestInfo(std::cout);
    std::cout.flush();
    test();
  }
  if (testResult)
  ...
  release();
  std::cout << Escape::Underline << Escape::TextBlue << "[Info] Test #" << gid
            << "." << id << " finished.\n";
}
void TestPoint::writeToTable(ResultTable& dest)
{
  ...
  dest.writeColumnList<ResultColumn, std::string&&>(
      {{ResultColumn::id, std::to_string(id)},
       {ResultColumn::runState,
        std::string(runResult[0]->name) +
            (runResult[1] ? " "s + runResult[1]->name : ""s)},
...
namespace {
enum class DataParam { TestRegex, Input, Answer };
...
enum class GroupParam { Verbose, Data, Basic, Use };
static const std::unordered_map<std::string, GroupParam> groupPar{
    ...
    {"use", GroupParam::Use}};
enum class DirParam { Path, Recursive, Regex };
...
TestGroup::TestGroup(const unsigned int id, const YAML::Node& node)
    : gid(id), results(ResultHeader, Escape::None)
{
  parseArgument(node);
}
TestGroup::TestGroup(const TestGroup& p, const unsigned int id,
...
void TestGroup::execute()
{
  ...
  tmpl.init();
  std::cout << Escape::TextBlue << Escape::Underline
            << "[Info] Start testing for group #" << gid;
  for (unsigned int i = 0; i < tests.size(); ++i) {
    std::cout.put('\n');
    TestPoint tst(i, gid, std::move(tests[i].first), std::move(tests[i].second),
                  tmpl);
    tst.execute(verbose);
    tst.writeToTable(results);
    summary.insert(fmt::format(FMT_STRING("{}.{}"), gid, i), tst);
  }
  std::cout << Escape::TextBlue << Escape::Underline << "[Info] Group #" << gid
            << " finished." << Escape::None << "\n\n";
}
void TestGroup::printResult(Testcase::Summary& totalSummary)
{
  std::cout << Escape::None << "Test result for group #" << gid << "\n";
  if (!tests.size()) {
    std::cout << Escape::TextRed << "[Err] Can't find any test data."
              << Escape::None << "\n";
    return;
  }
...
{
  unsigned int cur = 0;
  std::conditional_t<rec, fs::recursive_directory_iterator,
                     fs::directory_iterator>
      init(input.path);
  for (auto& i : init)
    if (i.is_regular_file()) {
      if (const auto [suc, hsh] = isInclude<rec>(i.path(), input.pattern);
          suc) {
...
template <bool rec>
std::pair<bool, std::string> TestGroup::isInclude(const fs::path& p,
                                                  const RegexSeq* r)
{
  std::string s;
  if constexpr (rec)
  ...
  if (!suc1) return std::make_pair(false, "");
  const auto [suc2, val2] = r->eval(s);
  if (!suc2) return std::make_pair(false, "");
...
void TestGroup::printConfig(GroupTable& dest)
{
  ...
  dest.writeColumnList<GroupColumn, std::string&&>(
      {{GroupColumn::id, std::to_string(gid)},
       {GroupColumn::inDir,
        input.rec ? input.path.string() + "(recursive)" : input.path.string()},
       {GroupColumn::ansDir, answer.rec ? answer.path.string() + "(recursive)"
                                        : answer.path.string()},
       {GroupColumn::timeLimit,
        fmt::format("{} ms ({} s)", tmpl.timeLimit / ms, tmpl.timeLimit / sec)},
       {GroupColumn::hardTimeLimit,
        fmt::format("{} ms ({} s)", tmpl.hardTimeLimit / ms,
                    tmpl.hardTimeLimit / sec)},
       {GroupColumn::memoryLimit,
        fmt::format("{} MiB ({} GiB)", tmpl.memoryLimit / mb,
                    tmpl.memoryLimit / gb)},
       {GroupColumn::hardMemoryLimit,
        fmt::format("{} MiB ({} GiB)", tmpl.hardMemoryLimit / mb,
                    tmpl.hardMemoryLimit / gb)},
...
int main(int argc, const char* argv[])
{
  ...
  if (version) Output::PrintVersion("group test runner", std::cout);
