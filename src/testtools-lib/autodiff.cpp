...
namespace {
enum class Param { Verbose, Limit, Diff, File };
...
AutoDiff& AutoDiff::instantiate(fmt::format_args args)
{
  ...
  if (differTemplate) differ = fmt::vformat(differTemplate, args);
  return *this;
}
AutoDiff& AutoDiff::instantiate()
{
  ...
  if (differTemplate) differ = differTemplate;
  return *this;
}
  ...
  if (!size) {
    if (verbose) {
      std::cout << Escape::TextCyan << "Autodiff: Redirect stdout to " << differ
                << "\n";
      std::cout << Escape::TextBlue << "Autodiff: Test command " << cmd
                << Escape::None << std::endl;
    }
    ...
    if (verbose) {
      std::cout << Escape::TextCyan << "Autodiff: File size limit " << size
                << " byte\n";
      if (exceed)
        std::cout << "Autodiff: File " << *exceed
                  << " too large, redirect stdout to file\n"
                  << "Autodiff: Redirect stdout to " << differ << "\n";
      std::cout << Escape::TextBlue << "Autodiff: Test command " << cmd
                << Escape::None << std::endl;
    }
    redirect = exceed;
    if (exceed) cmd.setRedirect(System::RedirectType::StdOut, differ);
  }
  if (!redirect) differ = "<unused>";
}
void AutoDiff::release()
{
  ...
  if (redirect) {
    if (Utility::removeFile(differ) && verbose)
      std::cout << Escape::TextGreen
                << "Autodiff: Test passed removed output file." << Escape::None
                << "\n";
  }
  else
    differ = "<unused>";
}
AutoDiff::~AutoDiff() { file.release(); }
}  // namespace apdebug::TestTools
