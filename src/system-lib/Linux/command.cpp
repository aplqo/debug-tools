...
Command::Command(Command&& other)
    : path(other.path), args(std::move(other.args))
{
...
Process Command::execute() const
{
  const pid_t p = vfork();
  if (!p) {
    if (fd[0] != -1) dup2(fd[0], STDIN_FILENO);
    if (fd[1] != -1) dup2(fd[1], STDOUT_FILENO);
    if (fd[2] != -1) dup2(fd[2], STDERR_FILENO);
...
void Command::parseArgument(const YAML::Node& node)
{
  for (const auto& it : node) {
    if (it.first.Scalar() == "path")
      ...
      templateArgs.parseArgument(it.second,
                                 [](const char** c, const YAML::Node& node) {
                                   *c = node.Scalar().c_str();
                                 });
  }
}
