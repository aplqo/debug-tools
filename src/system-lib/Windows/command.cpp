...
{
  instantiated = true;
  cmdline += Output::writeToString(
      " ", std::quoted<char, std::char_traits<char>>(arg));
  return *this;
}
Command& Command::instantiate(fmt::format_args args)
{
  instantiated = true;
  if (templateCmdline) cmdline = fmt::vformat(*templateCmdline, args);
...
{
  instantiated = true;
  if (templateCmdline) cmdline = *templateCmdline;
  std::copy(stdIO, stdIO + 3, stdioHandle);
  return *this;
}
Process Command::execute()
{
  ...
  UpdateProcThreadAttribute(info.lpAttributeList, 0,
                            PROC_THREAD_ATTRIBUTE_HANDLE_LIST, stdioHandle,
                            sizeof(stdioHandle), NULL, NULL);

  CreateProcessA(path.data(), const_cast<char*>(cmdline.data()), NULL, NULL,
                 true, EXTENDED_STARTUPINFO_PRESENT, NULL, NULL,
...
Command& Command::setRedirect(RedirectType typ,
                              const std::filesystem::path& file)
{
  ...
  switch (typ) {
    case RedirectType::StdIn:
      stdioHandle[0] = openFile[0] =
          CreateFileW(file.c_str(), GENERIC_READ,
                      FILE_SHARE_READ | FILE_SHARE_DELETE, &sec, OPEN_EXISTING,
                      FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
      break;
    case RedirectType::StdOut:
      stdioHandle[1] = openFile[1] = CreateFileW(
          file.c_str(), GENERIC_READ | GENERIC_WRITE, FILE_SHARE_DELETE, &sec,
          CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY, NULL);
      break;
    case RedirectType::StdErr:
      stdioHandle[2] = openFile[2] = CreateFileW(
          file.c_str(), GENERIC_WRITE | FILE_SHARE_READ | FILE_SHARE_DELETE,
  ...
  for (const auto& it : node)
    if (it.first.Scalar() == "path")
      ...
      for (const auto& com : it.second) {
        ...
        if (com.IsScalar())
          tmpl->append(
              Output::writeToString(std::quoted<char>(com.Scalar().c_str())));
        else {
          const auto& v = com["val"];
          if (com["quote"].as<bool>())
            tmpl->append(
                Output::writeToString(std::quoted<char>(v.Scalar().c_str())));
          else
...
std::ostream& operator<<(std::ostream& os, const Command& c)
{
  os << c.path;
  if (!c.instantiated) {
    if (c.templateCmdline) os << " " << *c.templateCmdline;
  }
  else
    os << " " << c.cmdline;
  return os;
