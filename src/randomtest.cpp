...
ResultTable results(std::array<const char*, 12>{"Id", "State(Run)",
                                                "State(Test)", "Input",
                                                "Output", "Answer", "Diff",
                                                "Real", "User", "System",
                                                "Memory", "Details"},
...
class TestPoint : public TestcaseType {
 public:
  TestPoint(const unsigned int tid, const unsigned int id,
            TestPointTemplate& tp);
  bool generate();
  bool run();
...
static const Testcase::Result judgeFail{.type = Testcase::Result::Type::Other,
                                        .name = "Fail",
                                        .color = Escape::None,
                                        .details = "Generate data failed"};

namespace {
enum class TemplateParam { Tmpdir, Generator, Standard, Validator };
...
void TestPointTemplate::print(std::ostream& os) const
{
  ...
  if (!tester.path.empty()) os << "[info] Test comman line: " << tester << "\n";
  os << static_cast<const Testcase::LimitInfo&>(*this) << "\n";
  os << Escape::None;
}

TestPoint::TestPoint(const unsigned int tid, const unsigned int id,
                     TestPointTemplate& tp)
    : TestcaseType((tp.tmpdir / fmt::format(FMT_COMPILE("{}-{}.in"),
                                            tp.platform->threadId, id))
                       .string(),
                   (tp.tmpdir / fmt::format(FMT_COMPILE("{}-{}.ans"),
                                            tp.platform->threadId, id))
                       .string(),
                   tp),
      tid(tid),
      ...
      standard(tp.standard)
{
  ...
  Replace::ReplaceStrict(
      fmt::make_format_args("input"_a = input, "output"_a = output,
                            "answer"_a = answer,
...
bool TestPoint::generate()
{
  ...
  do {
    generator.execute().wait();
    if (!tryRun(validor, fv)) continue;
    if (!tryRun(standard, fs)) continue;
    return true;
  } while (fv && fs);
...
void TestPoint::printTable(ResultTable& dest)
{
  ...
  dest.writeColumnList<ResultColumn, std::string&&>(
      {{ResultColumn::id, fmt::format("{}.{}", tid, id)},
       {ResultColumn::runState,
        std::string(runResult[0]->name) +
            (runResult[1] ? std::string(" ") + runResult[1]->name : "")},
       ...
       {ResultColumn::realTime,
        fmt::format(FMT_COMPILE("{}"), runTime.real / 1000.0)},
       {ResultColumn::userTime,
        fmt::format(FMT_COMPILE("{}"), runTime.user / 1000.0)},
       {ResultColumn::sysTime,
...
inline bool isRun()
{
  return testedCount.load(std::memory_order_consume) < times &&
         !(stop && fail.load(std::memory_order_consume));
}
struct Status {
  unsigned long lst;
...
void PrintThread<false>()
{
  Status stat{.lst = 0, .lastTime = begin};
  while (isRun()) {
    if (stat.printStatus()) std::cout.flush();
    std::this_thread::sleep_for(print_duration);
  }
  spentTime = System::Usage::getRealTime() - begin;
  std::cout << Escape::None << "\n";
...
void threadMain(const unsigned int tid)
{
  ...
  if constexpr (realTime) {
    const unsigned int tp = log10(times + 1) + 2, len = log10(tid + 1),
                       idl = plat.threadId.length();
    {
      std::lock_guard lk(tableLock);
  ...
  for (unsigned int i = 0; isRun(); ++i) {
    ...
    if (tst.generate()) tst.run();
    if (!tst.accept) fail = true;
    if (!tst.accept || showall) tst.printTable(local);
...
void setMinWidth()
{
  ...
  results.update(ResultColumn::mbMemory,
                 log10(global.hardMemoryLimit / 1024.0 + 1) + 5);
...
int main(int argc, const char* argv[])
{
  ...
  if (version) Output::PrintVersion("random test runner", std::cout);
  ...
  }
  {
    ...
    std::cout << Escape::TextBlue << "\n[Info] Test finished in " << time
              << "s. Average " << testedCount.load() / time << "/sec";
  }
  std::cout << Escape::None << "\n";
  return 0;
}
