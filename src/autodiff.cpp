...
namespace {
enum class Param { Version, Autodiff, Test };
...
int main(int argc, char* argv[])
{
  ...
  if (version) PrintVersion("auto diff", cout);
  if (diff.verbose)
    std::cout << Escape::TextCyan << "Autodiff: Using config file "
              << std::quoted(argv[1]) << "\n";
  diff.enable = true;
  ...
  if (!ret)
    ...
    cout << Escape::TextYellow << "Autodiff: Test program return " << ret
         << Escape::None << "\n";
  return ret;
}