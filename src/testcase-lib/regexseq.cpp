...
static constexpr auto regexFlag =
    std::regex_constants::ECMAScript | std::regex_constants::optimize;

struct operate {
  enum Type { Replace = 0x01, Extract = 0x02, Match = 0x03 } typ;
  union {
    struct {
      bool exact;
    } match;
...
namespace {
enum class Param { Type, Exact, Format, Time, Position };
  ...
  for (const auto& i : seq) {
    switch (i.typ) {
      case operate::Extract: {
        std::smatch sm;
        if (!(i.extract.exact ? std::regex_match(s, sm, reg)
                              : std::regex_search(s, sm, reg)))
      ...
      case operate::Match:
        if (!(i.match.exact ? std::regex_match(s, reg)
                            : std::regex_search(s, reg)))
          return std::make_pair(false, "");
        break;
      case operate::Replace:
        if (!i.replace.num)
          s = std::regex_replace(s, reg, i.replace.fmt,
...
{
  seq.parseArgument(node, [](OperateSeq* op, const YAML::Node& it) {
    new (op) OperateSeq{it};
  });
}
