...
void Stacktrace::parse(MemoryStream& ms)
{
  ...
  for (unsigned int i = 0; i < dump; ++i) {
    ...
    if (!userFrameDep && file == userFile) userFrameDep = i;
    if (fs::exists(file)) file = fs::relative(file, cur).string();
...
void Stacktrace::print(std::string& dest) const
{
  ...
  fmt::format_to(std::back_inserter(dest),
                 "\tStack backtrace: (dumped depth {} total depth {})",
...
const Result* parseWarn(MemoryStream& ms, Result*& cur)
{
  ...
  *cur = Result{.type = Result::Type::Warn,
  ...
  fmt::format_to(std::back_inserter(cur->verbose),
                 "[Warn] Interger overflow on operate {} of {}.", operate,
                 type);
  cur->verbose.push_back('\n');
  st.print(cur->verbose);
  cur->details = fmt::format("Interger overflow on {} of {} at {}", operate,
                             type, st.frames[st.userFrameDep]);
  return cur++;
}

const static Result UnknownExcept{.type = Result::Type::RE,
                                  .name = "RE",
...
const Result* parseFPE(MemoryStream& ms, Result*& cur)
{
  ...
  cur->verbose =
      "[RE] Received SIGFPE  erroneous arithmetic operation such as divide by "
      "zero.";
  cur->details.reserve(300 + st.maxFile + st.maxName + 60);
  cur->details = "Received SIGFPE";
  if (stat != Normal) cur->details += " (Float point exception:";
  if (stat & FPE::FE_Divbyzero) {
    cur->verbose +=
        "\n\tFE_DIVBYZERO: pole error occurred in an earlier floating-point "
        "operation.";
    cur->details += " FE_DIVBYZERO";
  }
  if (stat & FPE::FE_Inexact) {
    cur->verbose +=
        "\n\tFE_INEXACT: inexact result: rounding was necessary to store the "
        "result of an earlier floating-point operation.";
    cur->details += " FE_INEXACT";
  }
  if (stat & FPE::FE_Invavid) {
    cur->verbose +=
        "\n\tFE_INVALID: domain error occurred in an earlier floating-point "
        "operation";
    cur->details += " FE_INVALID";
  }
  if (stat & FPE::FE_Overflow) {
    cur->verbose +=
        "\n\tFE_OVERFLOW: the result of the earlier floating-point operation "
        "was too large to be representable";
    cur->details += " FE_OVERFLOW";
  }
  if (stat & FPE::FE_Underflow) {
    cur->verbose +=
        "\n\tFE_UNDERFLOW: the result of the earlier floating-point operation "
        "was subnormal with a loss of precision";
    cur->details += " FE_UNDERFLOW";
  }
  if (stat != Normal) cur->details += ")";
  fmt::format_to(std::back_inserter(cur->details), " at {}",
                 st.frames[st.userFrameDep]);
  cur->verbose.push_back('\n');
  st.print(cur->verbose);
  return cur++;
}
const Result* parseDivbyzero(MemoryStream& ms, Result*& cur)
{
  ...
  cur->name = "RE";
  cur->color = Output::Escape::TextPurple;
  cur->details = fmt::format("Divide by zero on {} at {}", type,
                             st.frames[st.userFrameDep]);
  cur->verbose.reserve(st.formatLength + 60);
...
const Result* parseRE(MemoryStream& ms, Result*& cur)
{
  ...
  switch (typ) {
    ...
    default:
      *cur = Result{.type = Result::Type::RE,
                    .name = "RE",
                    .color = Output::Escape::TextPurple,
