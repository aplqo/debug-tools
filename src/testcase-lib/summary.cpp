...
{
  if (val <= min.val) min = ValFrom{.val = val, .from = from};
  if (val >= max.val) max = ValFrom{.val = val, .from = from};
  sum += val;
}
template <class T>
...
{
  sum += other.sum;
  if (other.min.val <= min.val && other.min.from) min = other.min;
  if (other.max.val >= max.val && other.min.from) max = other.max;
}
template <class T>
...
std::string Summary::ValSummary<T>::format(const double cnt) const
{
  ...
  if (!min.from) return "<null>";
  return fmt::format(FMT_STRING("{} (#{})/ {} / {} (#{})"), round(min.val / u),
                     *min.from, round(sum / cnt / u), round(max.val / u),
                     *max.from);
}
void Summary::SummaryEntry::update(std::shared_ptr<std::string>& s,
                                   const TestResult& tst)
{
  ++count;
...
void Summary::print(std::ostream& os)
{
  ...
  for (unsigned int i = 0; i <= TypeNumber; ++i) {
    if (!entries[i].count) continue;
    tab.newColumn(i != TypeNumber ? color[i] : Output::Escape::None);
    tab.writeColumnList<int, std::string&&>(
        {{0, i == TypeNumber ? std::string("All") : std::string(name[i])},
         {1, fmt::format(FMT_STRING("{} ({}%)"), entries[i].count,
                         round((entries[i].count * 100.0) /
                               entries[TypeNumber].count))},
