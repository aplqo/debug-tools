...
class CheckedInteger {
 public:
  ...
  constexpr CheckedInteger(T s) : dat(s) {}
  template <class U>
  constexpr CheckedInteger(CheckedInteger<U> s)
  {
    if (s.dat > std::numeric_limits<T>::max()) err("assign");
    dat = s.dat;
  }
  template <class U>
  CheckedInteger& operator=(const CheckedInteger<U> v)
  {
    if (v.dat > std::numeric_limits<T>::max()) err("assign");
    dat = v.dat;
    return *this;
  }
  template <class U>
  ...
  {
    if (v > std::numeric_limits<T>::max()) err("assign");
    dat = v;
    return *this;
  }
  template <class U>
  ...
  {
    auto ret = dat + r;
    if (ret - dat != r || ((r > 0) ^ (ret > this->dat))) err("Add");
...
#define checkOp(op)                                           \
  template <class U>                                          \
  constexpr auto operator op(const CheckedInteger<U> r) const \
  {                                                           \
    return this->operator op(r.dat);                          \
  }
  checkOp(+);
  checkOp(-);
  checkOp(*);
  checkOp(/);
#undef checkOP
  constexpr operator T() const { return dat; };
  /*Increment and decrement*/
  inline CheckedInteger<T>& operator++()
  {
    T ret = dat;
    ++dat;
    if (ret >= dat) err("post-incresaement");
    return *this;
  }
  inline CheckedInteger<T> operator++(int t)
  {
    T ret = dat;
    ++dat;
    if (ret >= dat) err("pre-increasement");
    return ret;
  }
  inline CheckedInteger<T>& operator--()
  {
    T ret = dat;
    --dat;
    if (ret <= dat) err("post-decreasement");
    return *this;
  }
  inline CheckedInteger<T> operator--(int t)
  {
    T ret = dat;
    --dat;
    if (ret <= dat) err("pre-decreasement");
    return ret;
  }
#define oper(op)                              \
  template <class U>                          \
  constexpr auto operator op(const U r) const \
  {                                           \
    return CheckedInteger(dat op r);          \
  }
  oper(%);
...
#define oper(op)                                                     \
  template <class U>                                                 \
  constexpr inline bool operator op(const U r) const                 \
  {                                                                  \
    return dat op r;                                                 \
  }                                                                  \
  template <class U>                                                 \
  constexpr inline bool operator op(const CheckedInteger<U> r) const \
  {                                                                  \
    return dat op r.dat;                                             \
  }
...
#define assop(op)                              \
  template <class U>                           \
  inline CheckedInteger<T> operator op##=(U a) \
  {                                            \
...
#define assop(op)                                         \
  template <class U>                                      \
  inline auto operator op##=(const U dat)                 \
  {                                                       \
    return *this = this->operator op<U>(dat);             \
  }                                                       \
  template <class U>                                      \
  inline auto operator op##=(const CheckedInteger<U> dat) \
  {                                                       \
    return *this = this->operator op<U>(dat.dat);         \
  }
  assop(+);
  assop(-);
  assop(*);
  assop(/);
#undef assop
  constexpr auto operator~() const { return CheckedInteger(~dat); }
  template <class U>
