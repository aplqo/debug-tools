...
enum class Operate { Install, Uninstall, Init, Deinit, Update };
class compiler {
 public:
  compiler(const char* name, const char* descript)
      : name(name), description(descript)
  {
  }
  virtual void read() {}
  virtual void load(const std::filesystem::path& dest, const Operate op){};
  virtual void install(const std::filesystem::path& src,
                       const std::filesystem::path& dest);
  virtual void init(const std::filesystem::path& src,
                    const std::filesystem::path& dest);
  virtual void update(const std::filesystem::path& src,
                      const std::filesystem::path& dest);
 ...
 protected:
  virtual void initImpl(const std::filesystem::path& src,
...
class editor {
 public:
  editor(const char* name, const char* description)
      : name(name), description(description), com("Compiler")
  {
  }
